...
bits = sys.argv[2]
half = int(bits) // 2
path2 = path1.replace(bits, str(half))
...
with open(path1 + 'rsa%s.pem'%(bits), 'rb') as pub:
    pub = RSA.importKey(pub.read())

# Open up our known private keys
with open(path2 + 'rsa%d_priv.pem'%(half), 'rb') as priv:
    priv = RSA.importKey(priv.read())

with open(path2 + 'rsa%s_k1.txt'%(half), 'r') as k1:
    k1 = base64.b64decode(k1.read())

with open(path2 + 'rsa%s_k2.txt'%(half), 'r') as k2:
...
if found:
    n = p * q
    e = pub.e
    output = path1 + 'rsa%s_priv.pem'%(bits)
    phi = (p-1) * (q-1)
    d = modinv(e, phi)
    params = (n, e, d, p, q)
